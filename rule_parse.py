...
class Rule(object):
    ...
    def _replace(self, _str):
        ...
        if "[TIME]" not in _str:
            _r = self._url_parse()
            result = [_str.replace("[DOMAIN]", _r.get("domain", "")).replace("[HOST]", _r.get("host", "")).replace("[HOSTNAME]", _r.get("hostname", "ms"))]
        else:
            # 替换时间格式
            ...
            for x in range(0, self.timenum):
                ...
                strf_list = [
                    _str.replace("[TIME]", timedel.strftime('%Y-%m-%d')),
                    _str.replace("[TIME]", timedel.strftime('%Y%m%d')),
                    _str.replace("[TIME]", timedel.strftime('%Y_%m_%d'))
                ]
